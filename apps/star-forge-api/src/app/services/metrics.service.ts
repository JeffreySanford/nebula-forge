...
export class MetricsService {
  private metricsSubject: BehaviorSubject<SystemMetric[]> = new BehaviorSubject<SystemMetric[]>([]);
  ...
  private getRandomValue(type: string, currentValue: number): number {
    ...
    switch (type) {
      case 'cpu':
        return Math.min(Math.max(Math.round(currentValue + randomChange), 0), 100);
      case 'memory':
  ...
  getHistoricalMetrics(type: string, timespan?: number): SystemMetric[] {
    ...
    // In a real app, we would filter by timespan, but here we just return what we have
    return metrics;
  }
